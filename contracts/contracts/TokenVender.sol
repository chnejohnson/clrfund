...
contract TokenVender is ERC20, IUserRegistry, SimpleUserRegistry {
  ...
  uint256 public constant tokensPerAssetToken = 1;
  ...
  function redeemTokens(
    ...
    (bool fundsClaimed, , uint256 tallyResult) = _fundingRound.recipients(
      _voteOptionIndex
    );
    ...
    require(
      !tokensRedeemed[_voteOptionIndex],
      'TokenVender: Token already redeemed'
    );
    tokensRedeemed[_voteOptionIndex] = true;

    // 使用 round.getAllocatedAmount 來驗證可換取 dai 的 token 數量
    uint256 allocatedAmount = _fundingRound.getAllocatedAmount(
      tallyResult,
      _spent
    );
    ...
    require(
      ...
      'TokenVender: Sender is not the recipient'
    );
    ...
    emit TokensRedeemed(
      _voteOptionIndex,
      recipient,
      allocatedAmount * tokensPerAssetToken
    );
  }
